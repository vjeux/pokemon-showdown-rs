...
1. **Team Generator** (`tests/generate-test-teams.js`) - Generates random battle teams with a specific seed
2. **JavaScript Test Runner** (`tests/test-battle-js.js`) - Runs battles in JavaScript
3. **Rust Test Runner** (`examples/test_battle_rust.rs`) - Runs battles in Rust
4. **Comparison Script** (`tests/compare-battles.sh`) - Compares outputs line by line
...
**1. Generate test teams:**

```bash
node tests/generate-test-teams.js [seed_number]
```

This creates `tests/teams-seed[N].json` with:
- Random teams for both players
- Pokemon species, levels, moves, abilities, items
- EVs and IVs

**2. Run JavaScript battle:**
...
When tests fail, the comparison script shows:
