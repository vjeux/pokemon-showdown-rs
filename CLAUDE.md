...
All test scripts must be run from the `pokemon-showdown-rs` directory:
...
**Run all failing seeds tests:**
```bash
./tests/test-unified.sh --failing-seeds
```

This runs all minimal test cases, reports which bugs are fixed.
...
The `tests/minimized/` directory contains minimal reproduction cases for known bugs. Each file is a small team configuration that triggers a specific divergence between JS and Rust.

**Test Output Locations:**

The `test-minimized-seeds.sh` script outputs to several locations:
- **Console output** - Real-time progress and failures
- **`minimized-seeds-results.txt`** - Full log of all test results (in `pokemon-showdown-rs/`)
- **`tests/bug-analysis.txt`** - Analysis of patterns in failing seeds (moves, abilities, items)
...
### Debugging a Single Seed

The `compare-battles.sh` script supports both regular seeds and minimized seeds. Debug logging is enabled by default.

**Debug a regular seed:**
```bash
./tests/compare-battles.sh 42
```
This will generate teams using the random team generator and run the battle with full debug output.

**Debug a minimized seed:**
```bash
./tests/compare-battles.sh 65
```
If `tests/minimized/seed65.json` exists, it will automatically use the minimized team file instead of generating teams.

**Output files generated:**

| File | Description |
|------|-------------|
| `/tmp/teams-seed<N>-js.json` | Teams (JSON format) |
| `/tmp/teams-seed<N>-rust.json` | Teams (JSON format, copied from Docker) |
| `/tmp/js-battle-seed<N>.txt` | JS battle summary lines (used for comparison) |
| `/tmp/rust-battle-seed<N>.txt` | Rust battle summary lines (used for comparison) |
| `/tmp/js-battle-seed<N>-full.txt` | JS full battle log with detailed state |
| `/tmp/rust-battle-seed<N>-full.txt` | Rust full battle log with detailed state (includes debug logs) |
| `/tmp/js-battle-seed<N>-stdout.txt` | JS stdout only |
| `/tmp/js-battle-seed<N>-stderr.txt` | JS stderr only |

**View detailed logs:**
```bash
cat /tmp/js-battle-seed42-full.txt | less
cat /tmp/rust-battle-seed42-full.txt | less
```

**Compare logs side-by-side:**
```bash
diff /tmp/js-battle-seed42-full.txt /tmp/rust-battle-seed42-full.txt | less
```
...
```bash
cd /Users/vjeux/random/showdown/pokemon-showdown-rs && ./tests/test-unified.sh 1 500
```

6. For debugging specific failures, use the single seed comparison (debug logging enabled by default):
```bash
cd /Users/vjeux/random/showdown/pokemon-showdown-rs && ./tests/compare-battles.sh [seed_number]
```
