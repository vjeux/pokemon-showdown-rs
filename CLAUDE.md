...
5. **Comparison Script** (`tests/compare-battles.sh`) - Compares outputs in 4 steps:
   - Step 1: Generate teams in JavaScript → `/tmp/teams-seed*-js.json`
   - Step 2: Generate teams in Rust (in Docker) → `/tmp/teams-seed*-rust.json` (copied to host /tmp)
   - Step 3: Compare team generation (must match exactly!)
   - Step 4: Compare battle execution (if teams match)

**Important:** Each language generates its own teams to ensure the random team generation logic is also synchronized. Teams must match before battle testing can proceed. All generated files are stored in `/tmp` and not checked into git.
...
This creates:
- `/tmp/teams-seed[N]-js.json` - Teams generated by JavaScript (on host)
- `/tmp/teams-seed[N]-rust.json` - Teams generated by Rust (in container, then copied to host)

**2. Compare team generation:**

```bash
diff /tmp/teams-seed1-js.json /tmp/teams-seed1-rust.json
```
