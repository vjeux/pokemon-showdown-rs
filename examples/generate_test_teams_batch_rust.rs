/// Batch Rust Team Generator
///
/// Generates random battle teams for multiple seeds
///
/// Usage: cargo run --example generate_test_teams_batch_rust [start_seed] [end_seed]
/// Example: cargo run --example generate_test_teams_batch_rust 1 100

use pokemon_showdown::{Dex, PRNG, PRNGSeed, team_generator, dex_data::Gender};
use serde::Serialize;
use std::env;
use std::fs;
...
fn main() {
    ...
    eprintln!("Generating Rust teams for seeds {}-{}", start_seed, end_seed);

    let dex = Dex::load_default().expect("Failed to load dex");

    for seed_num in start_seed..=end_seed {
        match generate_teams(seed_num, &dex) {
            Ok(_) => {},
            Err(e) => {
    ...
    eprintln!("Rust team generation complete");
}
...
fn generate_teams(seed_num: u16, dex: &Dex) -> Result<(), String> {
