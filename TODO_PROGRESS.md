...
- Batch 181: 10 TODO comments (callback checking in find_pokemon_event_handlers)
  - Status conditions, volatiles, abilities, items, species, slot conditions
- Batch 182: 2 TODO comments (callback checking in find_side_event_handlers)
...
- Batch 188: 6 TODO comments (EffectState type system unification)
  - Unified dex_data::EffectState and event_system::EffectState into single type
  - Side and Field now use event_system::EffectState
  - Event handlers can now access full state data (duration, source, layers, etc.)
  - Fixed find_side_event_handlers.rs, find_field_event_handlers.rs, find_pokemon_event_handlers.rs
  - Updated add_side_condition.rs to create proper EffectState with source_slot as usize
  - Updated toxicdebris.rs to access layers from data HashMap
- Batch 189: 1 TODO (EffectState unification completion)
  - Fixed single_event_side.rs to set current_effect_state from side condition data
  - Updated state.rs to use event_system::EffectState for serialization
  - Eliminated all remaining uses of dex_data::EffectState
  - Codebase now exclusively uses event_system::EffectState
- Batch 190: 1 TODO (Species effectType clarification)
  - Resolved TODO questioning species effectType
  - Verified via JavaScript battle.ts that Condition is correct type for species
  - Added documentation explaining species use Condition effectType with subOrder 2
- Batch 191: 1 TODO (Format.team field)
  - Added missing team field to Format struct
  - Updated Format::from_def() and Format::non_existent() to set team field
  - Implemented format.team check in end_turn.rs ability validation
  - Format now matches JavaScript with team field for random battle formats
- Batch 192: 4 TODOs (use_move infrastructure for move callbacks)
  - Created Battle::use_move() wrapper method delegating to battle_actions::use_move::use_move
  - Created Battle::use_move_with_bounced() for reflected moves with hasBounced/pranksterBoosted flags
  - Made battle_actions::use_move module public for external access
  - Implemented use_move in copycat.rs (line 71)
  - Implemented use_move in sleeptalk.rs (line 144)
...
- Batch 168: 1 TODO (Sky Drop onFoeBeforeMove)
- Batch 169: 3 TODOs (Boost modification callbacks)
- Batch 170: 4 TODOs (Retaliation move target redirection)
...
- Batch 174: 1 TODO (NOT_FAIL handling)
- Batch 175: 3 TODOs (Side event infrastructure)
- Batch 176: 1 TODO (Pursuit onBeforeSwitchOut)
- Batch 177: 0 TODOs (infrastructure - 23 side condition callbacks wired)
...
- **Copycat** - ⭐ FULLY COMPLETE! (Batch 192)
  - onHit: Copies and executes the last move used by any Pokemon
- **Sleep Talk** - ⭐ FULLY COMPLETE! (Batch 192)
  - onTry: Requires asleep status or Comatose ability
  - onHit: Randomly executes one of the user's moves while asleep
- **Snatch** - ⭐ FULLY COMPLETE! (Batch 192)
  - condition::onStart: Displays Snatch status message
  - condition::onAnyPrepareHit: Steals snatchable moves and executes them
- **Magic Coat** - ⭐ FULLY COMPLETE! (Batch 192)
  - condition::onStart: Displays Magic Coat status and saves pranksterBoosted
  - condition::onTryHit: Reflects reflectable moves back to source
  - condition::onAllyTryHitSide: Reflects side-targeted moves in free-for-all battles
...
### Batch 179 - AfterSubDamage Damage Parameter (1 TODO)

**Files Modified**:
- `src/battle/handle_move_event.rs` - Extract damage from current_event.relay_var
- `src/data/move_callbacks/substitute.rs` - Use single_event_with_relay_var to pass actual_damage

**TODO Resolved**: 1 in handle_move_event.rs:
- Pass damage value to AfterSubDamage event handlers (line 66) - RESOLVED

**Problem**:

The AfterSubDamage event is triggered when a move hits a substitute. In JavaScript, the damage value is passed as the relay variable so moves can react to the damage dealt. The Rust implementation was hardcoding damage = 0.

**JavaScript Reference**:
```javascript
// In substitute's onTryPrimaryHit:
...
Modified substitute.rs to pass actual_damage through relay_var:
...
Modified handle_move_event.rs to extract damage from current_event:
...
**Effect**: Moves that use AfterSubDamage callbacks (like Rapid Spin in some generations) now receive the actual damage value when hitting a substitute, matching JavaScript behavior exactly.
...
### Batch 181 - Callback Checking in find_pokemon_event_handlers (10 TODO comments)

**Files Modified**:
- `src/battle/find_pokemon_event_handlers.rs` - Implemented callback and getKey checking for all effect types

**TODOs Resolved**: 10 TODO comments in find_pokemon_event_handlers.rs:
- Line 87-88: Check callback via getCallback and check getKey condition (status) - RESOLVED
- Line 111-112: Check callback via getCallback and check getKey condition (volatiles) - RESOLVED
- Line 133-134: Check callback via getCallback and check getKey condition (ability) - RESOLVED
- Line 154-155: Check callback via getCallback and check getKey condition (item) - RESOLVED
- Line 174: Check callback via getCallback (species) - RESOLVED
- Line 199-200: Check callback via getCallback and check getKey condition (slot conditions) - RESOLVED

**Problem**:

The find_pokemon_event_handlers function was adding event handlers for all effects (status, volatiles, abilities, items, species, slot conditions) without checking if the effect actually has a callback for the requested event. This could lead to unnecessary handler registration and execution.
...
if (callback !== undefined || (getKey && pokemon.statusState[getKey])) {
    handlers.push(this.resolvePriority({
        effect: status, callback, state: pokemon.statusState,
        end: pokemon.clearStatus, effectHolder: pokemon,
    }, callbackName));
}
```
...
Before (always added if effect exists):
```rust
...
After (checks callback existence and getKey condition):
```rust
if !pokemon.status.is_empty() {
...
Applied this pattern to all 6 effect categories:
1. **Status conditions** - Check has_callback for status and getKey in status_state
2. **Volatile conditions** - Check has_callback for each volatile and getKey in volatile_state
3. **Abilities** - Check has_callback for ability and getKey in ability_state
4. **Items** - Check has_callback for item and getKey in item_state
5. **Species** - Check has_callback for species (no getKey since species don't have duration)
6. **Slot conditions** - Check has_callback for slot condition and getKey in slot_condition_state

**Effect**: Event handler registration is now more efficient - only effects with actual callbacks for the event are registered. This matches JavaScript behavior exactly and reduces unnecessary event processing.
