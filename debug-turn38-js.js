...
battle.prng.rng.next = function() {
    ...
    // Capture Turn 38 calls (cumulative should be around 134-138 based on trace)
    if (totalPrngCalls >= 134 && totalPrngCalls <= 145) {
...
while (!battle.ended && turnNumber < 50) {
    ...
    if (turnNumber === 38) {
        ...
        for (const call of turn38Calls) {
            console.log(`  [PRNG #${call.num}] ${call.caller} -> ${call.value}`);
        }
