...
impl Pokemon {
    ...
    pub fn set_status(
        ...
        if !status.as_str().is_empty() {
            let status_effect = battle.make_status_effect(&status);
            let start_result = battle.single_event("Start", &status_effect, None, Some(pokemon_pos), source_pos, source_effect, None);
