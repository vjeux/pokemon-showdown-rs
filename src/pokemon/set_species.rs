...
impl Pokemon {
    /// Position-based set_species for use when you need to pass battle mutably
    /// This is the version that should be used from battle_actions and similar contexts
    ///
    /// JavaScript: this.setSpecies(rawSpecies, source)
    pub fn set_species_pos(
        battle: &mut Battle,
        ...
        // Note: ModifySpecies event not called - would need event infrastructure
        ...
        let (current_maxhp, current_status, gen) = {
            let pokemon = &battle.sides[pokemon_pos.0].pokemon[pokemon_pos.1];
            (pokemon.maxhp, pokemon.status.clone(), battle.gen)
        };
        ...
        pokemon.species_id = species_id.clone();
        ...
        pokemon.types = types.clone();
        pokemon.base_types = types.clone();
        pokemon.added_type = String::new();
        pokemon.apparent_type = types.join("/");
        ...
        if !is_transform {
            pokemon.base_stored_stats = StatsTable {
        ...
        if gen <= 1 {
            ...
            if current_status.as_str() == "par" {
                // Paralysis: Speed is 25% of normal
                pokemon.stored_stats.spe = ((pokemon.stored_stats.spe as f64) * 0.25) as i32;
            }
            if current_status.as_str() == "brn" {
                // Burn: Attack is 50% of normal
                pokemon.stored_stats.atk = ((pokemon.stored_stats.atk as f64) * 0.5) as i32;
            }
        }
    ...
    pub fn set_species(
        ...
        // JavaScript: this.addedType = species.addedType || '';
        self.added_type = String::new();
        // JavaScript: this.apparentType = rawSpecies.types.join('/');
        self.apparent_type = types.join("/");
