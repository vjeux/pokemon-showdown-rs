...
impl Battle {
    ...
    /// Check if move makes contact - uses ActiveMove directly
    ...
    // 	}
    pub fn check_move_makes_contact_with_active_move(
        &mut self,
        ...
        // Get the contact flag from the active move
        let has_contact = active_move
            .map(|m| m.flags.contact)
            .unwrap_or(false);
    ...
    /// Check if move makes contact - looks up from dex by move_id
    /// This is a fallback when ActiveMove is not available
    /// Equivalent to battle.ts checkMoveMakesContact()
    //
    // 	checkMoveMakesContact(move: ActiveMove, attacker: Pokemon, defender: Pokemon, announcePads = false) {
    // 		if (move.flags['contact'] && attacker.hasItem('protectivepads')) {
