...
impl Battle {
    ...
    pub fn shuffle_range<T>(&mut self, list: &mut [T], mut start: usize, end: usize) {
        debug_elog!("[SHUFFLE_RANGE] Called with start={}, end={}, turn={}", start, end, self.turn);
        while start < end - 1 {
            let next_index = self.random_with_range(start as i32, end as i32) as usize;
            debug_elog!("[SHUFFLE_RANGE] PRNG call: random_with_range({}, {}) = {}", start, end, next_index);
        ...
        debug_elog!("[SHUFFLE_RANGE] Complete, turn={}", self.turn);
    }
}
