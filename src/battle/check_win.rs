...
impl Battle {
    ...
    pub fn check_win(&mut self, faint_data: Option<FaintData>) -> bool {
        ...
        debug_elog!("[CHECK_WIN] Called at turn={}, faint_data={:?}", self.turn, faint_data.is_some());
        debug_elog!("[CHECK_WIN] Side 0 pokemon_left={}, Side 1 pokemon_left={}",
        ...
        if self.sides.iter().all(|side| side.pokemon_left == 0) {
            debug_elog!("[CHECK_WIN] All sides have 0 Pokemon left - tie/draw");
        ...
        for side_idx in 0..self.sides.len() {
            ...
            debug_elog!("[CHECK_WIN] Side {} foe_pokemon_left={}", side_idx, foe_pokemon_left);

            // JS: if (!side.foePokemonLeft())
            if foe_pokemon_left == 0 {
                debug_elog!("[CHECK_WIN] Side {} has won! (foe has 0 Pokemon left)", side_idx);
                // JS: this.win(side);
        ...
        debug_elog!("[CHECK_WIN] No winner yet");
        false
    }
}
