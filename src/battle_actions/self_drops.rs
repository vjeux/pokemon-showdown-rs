    ...
    for target in targets {
        ...
        if has_self_data && !self_dropped {
            ...
            if !is_secondary && has_boosts {
                ...
                // this.moveHit(source, source, move, moveData.self, isSecondary, true);
                // JavaScript calls moveHit which internally calls runMoveEffects
                // runMoveEffects applies all effects: boosts, status, volatileStatus, sideCondition, etc.
                // So we just need to call run_move_effects here - no manual effect application needed
