...
/// onModifyMove(move, source) {			if (!source.volatiles['skydrop']) {
/// 				move.accuracy = true;
...
/// ```text
/// JS Source (data/moves.ts):
/// onMoveFail(target, source) {			if (source.volatiles['twoturnmove'] && source.volatiles['twoturnmove'].duration === 1) {
...
/// ```text
/// JS Source (data/moves.ts):
/// onTryHit(target, source, move) {			if (source.removeVolatile(move.id)) {
