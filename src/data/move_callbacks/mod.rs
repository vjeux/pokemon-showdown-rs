...
/// Check if a move has an onHit callback
/// This is used by has_callback to check if a move has an onHit handler
/// that we can dispatch to
...
pub fn dispatch_condition_on_start(
    ...
    match condition_id {
        ...
        "dragoncheer" => dragoncheer::condition::on_start(battle, None, Some(pokemon_pos), None),
        "electrify" => electrify::condition::on_start(battle, Some(pokemon_pos)),
        "embargo" => embargo::condition::on_start(battle, pokemon_pos),
        "encore" => encore::condition::on_start(battle, Some(pokemon_pos)),
        "endure" => endure::condition::on_start(battle, Some(pokemon_pos)),
        "focusenergy" => focusenergy::condition::on_start(battle, None, Some(pokemon_pos), None),
        "focuspunch" => focuspunch::condition::on_start(battle, pokemon_pos),
        "followme" => followme::condition::on_start(battle, None, Some(pokemon_pos), None),
        "foresight" => foresight::condition::on_start(battle, pokemon_pos),
        "furycutter" => furycutter::condition::on_start(battle),
        "gastroacid" => gastroacid::condition::on_start(battle, pokemon_pos),
        "glaiverush" => glaiverush::condition::on_start(battle, pokemon_pos),
        "gmaxchistrike" => gmaxchistrike::condition::on_start(battle, Some(pokemon_pos), None, None),
        "grudge" => grudge::condition::on_start(battle, pokemon_pos),
        "healblock" => healblock::condition::on_start(battle, pokemon_pos, None),
        "helpinghand" => helpinghand::condition::on_start(battle, Some(pokemon_pos), None),
        "iceball" => iceball::condition::on_start(battle),
        "imprison" => imprison::condition::on_start(battle, Some(pokemon_pos)),
        "ingrain" => ingrain::condition::on_start(battle, pokemon_pos),
        "kingsshield" => kingsshield::condition::on_start(battle, Some(pokemon_pos)),
        "laserfocus" => laserfocus::condition::on_start(battle, pokemon_pos, None, None),
        "leechseed" => leechseed::condition::on_start(battle, Some(pokemon_pos)),
        "magiccoat" => magiccoat::condition::on_start(battle, Some(pokemon_pos), None, None),
        "magnetrise" => magnetrise::condition::on_start(battle, Some(pokemon_pos)),
        ...
        "obstruct" => obstruct::condition::on_start(battle, Some(pokemon_pos)),
        "octolock" => octolock::condition::on_start(battle, pokemon_pos, None),
        "powder" => powder::condition::on_start(battle, Some(pokemon_pos)),
        "powershift" => powershift::condition::on_start(battle, pokemon_pos),
        "powertrick" => powertrick::condition::on_start(battle, pokemon_pos),
        "protect" => protect::condition::on_start(battle, Some(pokemon_pos)),
        "rage" => rage::condition::on_start(battle, pokemon_pos),
        "ragepowder" => ragepowder::condition::on_start(battle, pokemon_pos),
        "rollout" => rollout::condition::on_start(battle),
        "roost" => roost::condition::on_start(battle, Some(pokemon_pos), None),
        "saltcure" => saltcure::condition::on_start(battle, pokemon_pos),
        "shelltrap" => shelltrap::condition::on_start(battle, pokemon_pos),
        "silktrap" => silktrap::condition::on_start(battle, Some(pokemon_pos)),
        "smackdown" => smackdown::condition::on_start(battle, pokemon_pos),
        "snatch" => snatch::condition::on_start(battle, pokemon_pos),
        "spikyshield" => spikyshield::condition::on_start(battle, Some(pokemon_pos)),
        "spotlight" => spotlight::condition::on_start(battle, pokemon_pos),
        "stockpile" => stockpile::condition::on_start(battle, Some(pokemon_pos)),
        "substitute" => substitute::condition::on_start(battle, Some(pokemon_pos), None, None),
        "syrupbomb" => syrupbomb::condition::on_start(battle, pokemon_pos),
        "tarshot" => tarshot::condition::on_start(battle, pokemon_pos),
        "taunt" => taunt::condition::on_start(battle, Some(pokemon_pos)),
        "telekinesis" => telekinesis::condition::on_start(battle, Some(pokemon_pos)),
        "throatchop" => throatchop::condition::on_start(battle, Some(pokemon_pos)),
        "torment" => torment::condition::on_start(battle, pokemon_pos, None, None),
        "uproar" => uproar::condition::on_start(battle, Some(pokemon_pos)),
        "wish" => wish::condition::on_start(battle, pokemon_pos, source_pos),
        "yawn" => yawn::condition::on_start(battle, Some(pokemon_pos), None),
        _ => EventResult::Continue,
    }
}
