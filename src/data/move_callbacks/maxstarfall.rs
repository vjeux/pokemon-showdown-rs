...
pub fn on_hit(battle: &mut Battle, _target_pos: (usize, usize), source_pos: Option<(usize, usize)>) -> EventResult {
    // onHit(source) { if (!source.volatiles['dynamax']) return; this.field.setTerrain('mistyterrain'); }
    ...
    // this.field.setTerrain('mistyterrain');
    let source_effect = battle.make_move_effect(&ID::from("maxstarfall"));
    battle.set_terrain(ID::from("mistyterrain"), source_pos, Some(source_effect));
