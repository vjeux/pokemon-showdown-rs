...
pub fn on_try_move(
    ...
    debug_elog!("[FREEZESHOCK_ONTRYMOVE] turn={}, pokemon=({},{})", battle.turn, source_pos.0, source_pos.1);
    ...
    let move_id = {
        let active_move = match &battle.active_move {
            ...
            None => {
                debug_elog!("[FREEZESHOCK_ONTRYMOVE] ERROR: active_move is None, returning Continue");
    ...
    let has_volatile = {
        ...
        debug_elog!("[FREEZESHOCK_ONTRYMOVE] Checking for volatile '{}'. has_volatile={}", move_id.as_str(), pokemon.has_volatile(&move_id));
        debug_elog!("[FREEZESHOCK_ONTRYMOVE] All volatiles: {:?}", pokemon.volatiles.keys().map(|k| k.as_str()).collect::<Vec<_>>());
        pokemon.has_volatile(&move_id)
    };

    if has_volatile {
        debug_elog!("[FREEZESHOCK_ONTRYMOVE] Found volatile, removing and returning Continue");
