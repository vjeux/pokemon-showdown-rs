...
/// onModifyMove(move, pokemon, target) {			if (pokemon.volatiles['iceball'] || pokemon.status === 'slp' || !target) return;
...
/// ```text
/// JS Source (data/moves.ts):
/// onAfterMove(source, target, move) {			const iceballData = source.volatiles["iceball"];
/// 			if (
/// 				iceballData &&
