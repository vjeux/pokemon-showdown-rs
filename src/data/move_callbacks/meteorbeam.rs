...
pub fn on_try_move(
    ...
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] turn={}, source=({},{}), target={:?}",
    ...
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] move_id={}, removed={}", move_id.as_str(), removed);

    if removed {
        // return;
        debug_elog!("[METEORBEAM_ON_TRY_MOVE] Volatile removed, returning Continue (2nd turn execution)");
        return EventResult::Continue;
    }

    debug_elog!("[METEORBEAM_ON_TRY_MOVE] Past removeVolatile check, preparing charge turn");
    ...
    let (attacker_arg, move_name) = {
        let attacker_pokemon = match battle.pokemon_at(attacker.0, attacker.1) {
            Some(p) => p,
            None => {
                debug_elog!("[METEORBEAM_ON_TRY_MOVE] Attacker pokemon not found, returning Continue");
                return EventResult::Continue;
            }
        };
    ...
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] Adding -prepare message");
    battle.add("-prepare", &[attacker_arg.into(), move_name.into()]);

    // this.boost({ spa: 1 }, attacker, attacker, move);
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] Calling boost for SpA+1");
    battle.boost(
        &[("spa", 1)],
        attacker,
        Some(attacker),
    ...
    //     return;
    // }
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] Running ChargeMove event");
    ...
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] ChargeMove result: {:?}", charge_result);
    // JavaScript: if (!result) return;
    // This checks if result is FALSY, not if it equals 0
    if !charge_result.is_truthy() {
        debug_elog!("[METEORBEAM_ON_TRY_MOVE] ChargeMove returned falsy, returning Continue");
        return EventResult::Continue;
    }

    // attacker.addVolatile('twoturnmove', defender);
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] About to add twoturnmove volatile");
    Pokemon::add_volatile(battle, attacker, ID::from("twoturnmove"), defender, Some(&Effect::move_(move_id)), None, None);
    debug_elog!("[METEORBEAM_ON_TRY_MOVE] Added twoturnmove volatile, returning Stop (charge turn)");
