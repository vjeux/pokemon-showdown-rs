...
/// onPrepareHit(target, source, move) {			if (!source.isAlly(target)) {
...
/// ```text
/// JS Source (data/moves.ts):
/// onModifyMove(move, pokemon, target) {			if (!target) return;
...
/// ```text
/// JS Source (data/moves.ts):
/// onHit(target, source, move) {			// Shell Side Arm normally reveals its category via animation on cart, but doesn't play either custom animation against allies
