...
pub fn on_source_modify_damage(battle: &mut Battle, _damage: i32, _source_pos: (usize, usize), target_pos: (usize, usize)) -> EventResult {
    ...
    // Check if move.type === 'Grass'
    let (is_grass_type, bypass_sub, infiltrates) = match &battle.active_move {
        Some(active_move) => (
    ...
    let type_effectiveness = {
        ...
        Pokemon::run_effectiveness(battle, target_pos, &ID::from("Grass"))
    };
    ...
    if item_eaten.is_some() {
        ...
        battle.add("-enditem", &[target_ident.as_str().into(), "Rindo Berry".into(), "[weaken]".into()]);
