...
pub fn on_residual(battle: &mut Battle, pokemon_pos: (usize, usize)) -> EventResult {
    ...
    let should_eat = {
        if let Some(pokemon) = battle.pokemon_at(pokemon_pos.0, pokemon_pos.1) {
            ...
            debug_elog!("[MICLEBERRY_RESIDUAL] pokemon={:?}, hp={}, maxhp={}, quarter_hp={}, half_hp={}",
                      pokemon_pos, pokemon.hp, pokemon.maxhp, quarter_hp, half_hp);

            if pokemon.hp <= quarter_hp {
                debug_elog!("[MICLEBERRY_RESIDUAL] hp <= quarter_hp, should_eat=true");
                true
                ...
                debug_elog!("[MICLEBERRY_RESIDUAL] hp > half_hp, should_eat=false");
                false
            }
        } else {
            false
