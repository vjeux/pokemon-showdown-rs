...
pub fn dispatch_on_source_modify_damage(
    ...
    match item_id {
        "babiriberry" => babiriberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "chartiberry" => chartiberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "chilanberry" => chilanberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "chopleberry" => chopleberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "cobaberry" => cobaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "colburberry" => colburberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "habanberry" => habanberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "kasibberry" => kasibberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "kebiaberry" => kebiaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "occaberry" => occaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "passhoberry" => passhoberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "payapaberry" => payapaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "rindoberry" => rindoberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "roseliberry" => roseliberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "shucaberry" => shucaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "tangaberry" => tangaberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "wacanberry" => wacanberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        "yacheberry" => yacheberry::on_source_modify_damage(battle, damage, source_pos, target_pos, active_move),
        _ => EventResult::Continue,
    }
}
...
pub fn dispatch_on_take_item(
    ...
    match item_id {
        ...
        _ => {
            ...
            crate::debug_elog!("[DISPATCH_ON_TAKE_ITEM] item_id={}", item_id);
            if let Some(item_data) = battle.dex.items().get_by_id(&item_id.into()) {
                crate::debug_elog!("[DISPATCH_ON_TAKE_ITEM] item_data found, z_move={:?}, z_move.is_some()={}", item_data.z_move, item_data.z_move.is_some());
                if item_data.z_move.is_some() {
                    crate::debug_elog!("[DISPATCH_ON_TAKE_ITEM] Returning Boolean(false) to block Z-crystal removal");
                    return EventResult::Boolean(false);
                }
            } else {
                crate::debug_elog!("[DISPATCH_ON_TAKE_ITEM] item_data NOT found for {}", item_id);
            }
            crate::debug_elog!("[DISPATCH_ON_TAKE_ITEM] Returning Continue (allowing item removal)");
