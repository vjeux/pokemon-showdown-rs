...
pub fn on_source_modify_damage(battle: &mut Battle, _damage: i32, _source_pos: (usize, usize), target_pos: (usize, usize)) -> EventResult {
    ...
    // Check if move.type === 'Fairy'
    let (is_fairy_type, bypass_sub, infiltrates) = match &battle.active_move {
        Some(active_move) => (
            active_move.move_type == "Fairy",
    ...
    let type_effectiveness = {
        ...
        Pokemon::run_effectiveness(battle, target_pos, &ID::from("Fairy"))
    };
    ...
    if item_eaten.is_some() {
        ...
        battle.add("-enditem", &[target_ident.as_str().into(), "Roseli Berry".into(), "[weaken]".into()]);
