...
pub fn on_foe_try_move(battle: &mut Battle, target_pos: Option<(usize, usize)>, source_pos: Option<(usize, usize)>, active_move: Option<&crate::battle_actions::ActiveMove>) -> EventResult {
    ...
    // const dazzlingHolder = this.effectState.target;
    let dazzling_holder = match battle.effect_state.borrow().target {
        Some(pos) => pos,
        None => return EventResult::Continue,
    };

    // if ((source.isAlly(dazzlingHolder) || move.target === 'all') && move.priority > 0.1)
    let source_is_ally = source_pos.map_or(false, |src_pos| battle.is_ally(src_pos, dazzling_holder));
    ...
    if (source_is_ally || move_target == "all") && move_priority as f32 > 0.1 {
        ...
        // this.add('cant', dazzlingHolder, 'ability: Dazzling', move, `[of] ${target}`);
        let dazzling_slot = {
            let pokemon = match battle.pokemon_at(dazzling_holder.0, dazzling_holder.1) {
                Some(p) => p,
