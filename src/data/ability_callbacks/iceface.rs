...
pub fn on_start(battle: &mut Battle, pokemon_pos: (usize, usize), _source_pos: Option<(usize, usize)>, _effect: Option<&Effect>) -> EventResult {
    ...
    if let Some(pokemon) = battle.pokemon_at_mut(pokemon_pos.0, pokemon_pos.1) {
        pokemon.ability_state.borrow_mut().busted = Some(false);
    }
...
pub fn on_damage(battle: &mut Battle, _damage: i32, target_pos: (usize, usize), _source_pos: Option<(usize, usize)>, effect: Option<&Effect>) -> EventResult {
    ...
    if let Some(pokemon) = battle.pokemon_at_mut(target_pos.0, target_pos.1) {
        pokemon.ability_state.borrow_mut().busted = Some(true);
    }
...
pub fn on_update(battle: &mut Battle, pokemon_pos: (usize, usize)) -> EventResult {
    ...
    let (species_id, busted) = {
        ...
        let is_busted = pokemon.ability_state.borrow().busted.unwrap_or(false);
