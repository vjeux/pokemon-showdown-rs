...
pub fn on_any_modify_sp_a(battle: &mut Battle, _spa: i32, source_pos: Option<(usize, usize)>, target_pos: Option<(usize, usize)>, _active_move: Option<&crate::battle_actions::ActiveMove>) -> EventResult {
    ...
    let (_current_ruined, need_to_update) = {
        if let Some(ref active_move) = battle.active_move {
            if let Some(ruined_pos) = active_move.borrow().ruined_spa {
    ...
    if need_to_update {
        if let Some(ref active_move) = battle.active_move {
            active_move.borrow_mut().ruined_spa = Some(ability_holder);
        }
    }

    let should_apply = {
        if let Some(ref active_move) = battle.active_move {
            active_move.borrow().ruined_spa == Some(ability_holder)
        } else {
            false
        }
    };
