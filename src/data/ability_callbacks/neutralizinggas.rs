...
pub fn on_switch_in(battle: &mut Battle, pokemon_pos: (usize, usize)) -> EventResult {
    ...
    for target_pos in active_positions {
        ...
        if has_illusion {
            // this.singleEvent('End', this.dex.abilities.get('Illusion'), target.abilityState, target, pokemon, 'neutralizinggas');
            battle.single_event("End", &crate::battle::Effect::ability(ID::from("illusion")), Some(target_pos), Some(pokemon_pos), Some(&crate::battle::Effect::ability(ID::from("neutralizinggas"))), None);
        }
        ...
        if strong_weathers.contains(&target_ability_id.as_str()) {
            // this.singleEvent('End', this.dex.abilities.get(target.getAbility().id), target.abilityState, target, pokemon, 'neutralizinggas');
            battle.single_event("End", &crate::battle::Effect::ability(ID::from(target_ability_id.as_str())), Some(target_pos), Some(pokemon_pos), Some(&crate::battle::Effect::ability(ID::from("neutralizinggas"))), None);
        }
    }
