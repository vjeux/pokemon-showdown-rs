...
pub fn on_damaging_hit(battle: &mut Battle, _damage: i32, target_pos: Option<(usize, usize)>, source_pos: Option<(usize, usize)>, _active_move: Option<&crate::battle_actions::ActiveMove>) -> EventResult {
    ...
    if has_illusion {
        ...
        battle.single_event("End", &battle.make_ability_effect(&illusion_id), None, Some(target), source_pos, None, None);
    }
...
///     pokemon.illusion = null;
/// }
pub fn on_faint(battle: &mut Battle, pokemon_pos: (usize, usize), _source_pos: Option<(usize, usize)>, _effect: Option<&Effect>) -> EventResult {
    // pokemon.illusion = null;
    let pokemon = match battle.pokemon_at_mut(pokemon_pos.0, pokemon_pos.1) {
        Some(p) => p,
        None => return EventResult::Continue,
